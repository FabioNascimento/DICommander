...
namespace UILibrary
{
    ...
    {
        private int previouslastline = -1; //ADDED BY HN
        ...
        private void printPrompt()
        {
            ...
            if (currentText.Length != 0 && currentText[currentText.Length - 1] != '\n')
                printLine();
            previouslastline = this.Lines.Length - 2;//ADDED BY HN
            this.AddText(prompt);
        }
        ...
        private void shellTextBox_KeyPress(object sender, System.Windows.Forms.KeyPressEventArgs e)
        {
            ...
            if (IsTerminatorKey(e.KeyChar))
            {
                ...
                foreach (string command in currentCommands)
                {
                    ...
                    {
                        printLine();
                        ((ShellControl)this.Parent).FireCommandEntered(cmd);
        ...
        private string[] GetCurrentLines()
        {
            ...
            int linecount = currentlastline - (previouslastline<0?-1:previouslastline);
            string[] result = new string[linecount];
            int resultline = 0;//HN
            for (int lineno = (previouslastline<0?0:previouslastline+1); lineno <= currentlastline; lineno++)
            {
                result[resultline] = (string)this.Lines.GetValue(lineno);
                resultline++;
            }
            return result;
        }
