...
namespace IronPython.Modules
{
    ...
    public static partial class Builtin
    {
        ...
        {
            DICEngine engine = DICEngine.GetInstance();
            return engine.DefaultSession.Session;
        }

        [PythonName("get")]
        public static object Get(BoObjectTypes objtype)
        {
            CompanyClass session = GetSession();
            if (session != null)
            {
                return session.GetBusinessObject(objtype);
            }
            else
            {
                return null;
        ...
        [PythonName("get")]
        public static object Get(BoObjectTypes objtype, object key)
        {
            CompanyClass session = GetSession();
            if (session != null)
            {
                ...
                Type t = o.GetType();
                object res = t.InvokeMember("GetByKey", BindingFlags.InvokeMethod, null, o, new object[] { key });
                return o;
            }
            else
            {
        ...
        [PythonName("get")]
        public static object Get(BoObjectTypes objtype, object key, CompanyClass session)
        {
            if (session != null)
            {
                object o = session.GetBusinessObject(objtype);
                Type t = o.GetType();
                t.InvokeMember("GetByKey", BindingFlags.InvokeMethod, null, o, new object[] { key });
                return o;
            }
            else
            {
        ...
        [PythonName("get")]
        public static object Get(BoObjectTypes objtype, CompanyClass session)
        {
            return session.GetBusinessObject(objtype);
        }

        [PythonName("remove")]
        public static bool Remove(BoObjectTypes objtype, object key)
        {
            CompanyClass session = GetSession();
            if (session != null)
            {
                object o = Get(objtype, session);
                Type t = o.GetType();
                t.InvokeMember("GetByKey", BindingFlags.InvokeMethod, null, o, new object[] { key });
                object res = t.InvokeMember("Remove", BindingFlags.InvokeMethod, null, o, new object[] { });
                if (DICEngine.AutoRelease)
                {
            ...
            {
                return false; //FIXME: IMPLEMENT BETTER EXCEPTION HANDLING
            }
        }

        [PythonName("remove")]
        public static bool Remove(BoObjectTypes objtype, object key, CompanyClass session)
        {
            object o = session.GetBusinessObject(objtype);
            Type t = o.GetType();
            t.InvokeMember("GetByKey", BindingFlags.InvokeMethod, null, o, new object[] { key });
        ...
        [PythonName("starttransaction")]
        public static void StartTransaction(CompanyClass session)
        {
            session.StartTransaction();
        }
        ...
        {
            CompanyClass session = GetSession();
            session.EndTransaction(BoWfTransOpt.wf_Commit);
        }

        [PythonName("commit")]
        public static void Commit(CompanyClass session)
        {
            session.EndTransaction(BoWfTransOpt.wf_Commit);
        }
        ...
        {
            CompanyClass session = GetSession();
            session.EndTransaction(BoWfTransOpt.wf_RollBack);
        }

        [PythonName("rollback")]
        public static void Rollback(CompanyClass session)
        {
            session.EndTransaction(BoWfTransOpt.wf_RollBack);
        }

        [PythonName("geterrorcode")]
        ...
        [PythonName("geterrordescription")]
        ...
        {
            Type t = o.GetType();
            object res = t.InvokeMember("Update", BindingFlags.InvokeMethod, null, o, new object[] { });
            if (DICEngine.AutoRelease)
            {
                System.Runtime.InteropServices.Marshal.FinalReleaseComObject(o);
            }
        ...
        [PythonName("cancel")]
        public static bool Cancel(object o)
        {
            Type t = o.GetType();
            object res = t.InvokeMember("Cancel", BindingFlags.InvokeMethod, null, o, new object[] { });
            if (DICEngine.AutoRelease)
            {
                System.Runtime.InteropServices.Marshal.FinalReleaseComObject(o);
            }
        ...
        [PythonName("close")]
        public static bool Close(object o)
        {
            Type t = o.GetType();
            object res = t.InvokeMember("Close", BindingFlags.InvokeMethod, null, o, new object[] { });
            if (DICEngine.AutoRelease)
            {
                System.Runtime.InteropServices.Marshal.FinalReleaseComObject(o);
            }
        ...
        {
            Type t = o.GetType();
            object res = t.InvokeMember("Add", BindingFlags.InvokeMethod, null, o, new object[] { });
            if (DICEngine.AutoRelease)
            {
                System.Runtime.InteropServices.Marshal.FinalReleaseComObject(o);
            }
            //res is always null when handling a line object, thus true
        ...
        [PythonName("insert")]
        public static bool Insert(BoObjectTypes objtype, Fields data)
        {
            CompanyClass session = GetSession();
            return Insert(objtype, data, session);
        }

        [PythonName("insert")]
        public static bool Insert(BoObjectTypes objtype, Fields data, CompanyClass session)
        {
            object o = Get(objtype, session);
            Merge(o, data);
            return Add(o);
        ...
        [PythonName("upsert")]
        public static bool Upsert(BoObjectTypes objtype, Fields data)
        {
            CompanyClass session = GetSession();
            return Upsert(objtype, data, session);
        }

        [PythonName("upsert")]
        public static bool Upsert(BoObjectTypes objtype, Fields data, CompanyClass session)
        {
            object o = Get(objtype);
            Type t = o.GetType();
            object res = (bool)t.InvokeMember("GetByKey", BindingFlags.InvokeMethod, null, o, new object[] { data.Item(0).Value });
            bool isupdate = res != null && res.Equals(true);
            Merge(o, data);
            if (isupdate)
            {
                return Update(o);
        ...
        [PythonName("getmerge")]
        public static object GetMerge(BoObjectTypes objtype, Fields data)
        {
            CompanyClass session = GetSession();
            return GetMerge(objtype, data, session);
        }

        [PythonName("getmerge")]
        public static object GetMerge(BoObjectTypes objtype, Fields data, CompanyClass session)
        {
            object o = Get(objtype, data.Item(0).Value, session);
            Merge(o, data);
            return o;
        }

        [PythonName("merge")]
        public static object Merge(object o, Fields data)
        {
            Type t = o.GetType();
            ...
            foreach (Field field in data)
            {
                try
                {
                    object res = t.InvokeMember(field.Name, BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.SetField | BindingFlags.SetProperty, null, o, new object[] { field.Value });
                    success++;
                }
                catch (Exception ex)
                {
            ...
            if (success > 0)
            {
                sb.Append("successfully mapped " + success + " columns.");
            }
            if (failure > 0)
            {
                sb.Append("Failed to map " + failure + "columns.");
            }
            return sb.ToString();
        }
        ...
        [PythonName("browse")]
        public static IEnumerable<object> Browse(BoObjectTypes objtype, Recordset rs)
        {
            object o = Get(objtype);
        ...
        [PythonName("browse")]
        public static IEnumerable<object> Browse(BoObjectTypes objtype, Recordset rs, CompanyClass session)
        {
            object o = session.GetBusinessObject(objtype);
            return new BOBSObjectRecordSetBrowser(o, rs);
        }
        ...
        [PythonName("browse")]
        public static IEnumerable<object> Browse(Recordset rs, string columnname)
        {
        ...
        /// </summary>
        class Browser : IEnumerable<object>
        {
            object lines;
            Type t;
        ...
        /// </summary>
        class BOBSObjectRecordSetBrowser : IEnumerable<object>
        {
            object o;
            Recordset rs;
            Type t;
            public BOBSObjectRecordSetBrowser(object _o, Recordset _rs)
            {
                o = _o;
                rs = _rs;
                t = o.GetType();
            ...
            public IEnumerator<object> GetEnumerator()
            {
                bool originalAutoRelease = DICEngine.AutoRelease;
                DICEngine.AutoRelease = false;
                try
                {
                ...
                {
                    DICEngine.AutoRelease = originalAutoRelease;
                }
            }
        }
        ...
        /// </summary>
        class ColumnBrowser : IEnumerable<object>
        {
            string column;
            Recordset rs;
        ...
        /// </summary>
        class RowBrowser : IEnumerable<object>
        {
            Recordset rs;
